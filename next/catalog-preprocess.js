(() => {
  ...
  const COLOR_NAMES = [
    'светло-серый','светло-серый','темно-серый','темно-серый','слоновая кость',
    ...
    'графит','серебристый','золотой','натуральный','орех','дуб','хром','венге'
  ];
  ...
  window.fetch = async function formaCatalogFetch(input, init) {
    ...
    try {
      ...
      console.warn('[catalog-preprocess] using original catalog', error);
      return response;
    }
  };

  function preprocess(products) {
    const prepared = products.map(product => enrichColors(structuredCloneSafe(product)));
    return mergeKnownGland(prepared);
  }

  function enrichColors(product) {
    if (Array.isArray(product.colors)) {
      product.colors.forEach(group => {
        const variants = Array.isArray(group.variants) ? group.variants : [];
        variants.forEach(variant => enrichVariantColor(variant));
        if (!String(group.label || '').trim()) {
          const labels = unique(variants.map(variant => variant.color || colorFromSpecs(variant.specs)).filter(Boolean));
          if (labels.length === 1) group.label = labels[0];
        }
      });
    }
  ...
  function mergeKnownGland(products) {
    ...
    matches.flatMap(flattenVariants).forEach(variant => {
      ...
      const signature = [
        normalizeSpecs(copy.specs),
        Number(copy.wholesalePrice) || 0,
        Number(copy.retailPrice) || 0,
        normalize(copy.color || colorFromSpecs(copy.specs)),
      ].join('|');
      ...
      if (!existing) {
        ...
        existing.images = unique([...(existing.images || []), ...(copy.images || []), copy.image, copy.directImage].filter(Boolean));
      }
    });
    ...
    if (Array.isArray(product.colors)) {
      product.colors.forEach(group => {
        (Array.isArray(group.variants) ? group.variants : []).forEach(variant => {
          variants.push({ ...variant, color: variant.color || group.label || '' });
        });
      });
    }
  ...
  function normalizeSpecs(value) {
    return normalize(value)
      .replace(/\(?\d+\s*шт\.?\s*(?:в\s*)?упаковк[еи]?\)?/giu, '')
      .replace(/\s+/g, ' ')
      .trim();
  }
