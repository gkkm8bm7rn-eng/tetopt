...
def run_case(pw,label,url,engine,device):
    ...
    page=context.new_page(); errors=[]; console=[]; checks={}
    ...
    try:
        ...
        # Sort controls.
        page.locator('#sortSelect').select_option('price-asc'); page.wait_for_timeout(150)
        prices=page.locator('#productGrid .product-price').evaluate_all("xs=>xs.map(x=>Number(x.textContent.replace(/\\D/g,'')))")
        checks['sort_price_asc']=prices==sorted(prices)
        page.locator('#sortSelect').select_option('price-desc'); page.wait_for_timeout(150)
        prices2=page.locator('#productGrid .product-price').evaluate_all("xs=>xs.map(x=>Number(x.textContent.replace(/\\D/g,'')))")
        checks['sort_price_desc']=prices2==sorted(prices2,reverse=True)
        page.locator('#sortSelect').select_option('featured')

        # Price filter and reset.
        page.locator('#priceMin').fill('10000'); page.wait_for_timeout(150)
        vals=page.locator('#productGrid .product-price').evaluate_all("xs=>xs.map(x=>Number(x.textContent.replace(/\\D/g,'')))")
        checks['price_filter']=bool(vals) and min(vals)>=10000
        page.locator('#clearFilters').click(); page.wait_for_timeout(150)
        checks['filter_reset']=page.locator('#priceMin').input_value()==''

        # Multiple variants filter.
        page.locator('#multiVariant').check(); page.wait_for_timeout(150)
        checks['multi_variant_filter']=page.locator('#productGrid .product-card').count()>0
        page.locator('#multiVariant').uncheck(); page.wait_for_timeout(100)

        # Category and subcategory navigation.
        page.locator(".category-primary [data-main-category='interior']").click(); page.wait_for_timeout(100)
        sub=page.locator("[data-category-code='1']").first
        sub.click(); page.wait_for_timeout(150)
        checks['subcategory']=page.locator('#catalogTitle').inner_text().strip()=='Стулья'

        # Search source article from first card after reset via main all.
        page.locator(".category-primary [data-main-category='all']").click(); page.wait_for_timeout(100)
        sid=page.locator('#productGrid .quick-add').first.get_attribute('data-source')
        page.locator('#searchInput').fill(sid); page.locator('#searchForm .search-submit').click(); page.wait_for_timeout(150)
        checks['article_search']=page.locator('#productGrid .product-card').count()>=1
        page.locator('#searchInput').fill(''); page.locator('#searchForm .search-submit').click(); page.wait_for_timeout(150)

        # Correct pagination selector (excludes next arrow).
        ...
        page.locator("#pagination .page-button:not(.page-arrow)[data-page='1']").click(); page.wait_for_timeout(150)

        # Favorite + favorites view + share control.
        fav=page.locator('#productGrid .favorite').first; fav.click(); page.wait_for_timeout(100)
        ...
        page.locator(".category-primary [data-main-category='all']").click(); page.wait_for_timeout(150)
        add=page.locator('#productGrid .quick-add').first; add.click(); page.wait_for_timeout(150)
        checks['cart_count']=int(page.locator('#cartCount').inner_text())>=1
        qty_plus=page.locator("#productGrid [data-card-cart-delta='1']").first
        if qty_plus.count():
            before=int(page.locator('#cartCount').inner_text()); qty_plus.click(); page.wait_for_timeout(100); checks['card_cart_plus']=int(page.locator('#cartCount').inner_text())==before+1
        ...
        remove=page.locator('#cartItems [data-remove]').first; remove.click(); page.wait_for_timeout(150)
        checks['cart_remove']=int(page.locator('#cartCount').inner_text())>=0
        ...
        if alt.count(): alt.first.click(); page.wait_for_timeout(250); checks['detail_variant']=True
        ...
        return {'label':label,'checks':checks,'false':[k for k,v in checks.items() if v is False],'page_errors':errors,'console_errors':console[:10]}
    except Exception as e:
        return {'label':label,'fatal':repr(e),'checks':checks,'page_errors':errors,'console_errors':console[:10]}
    finally:
