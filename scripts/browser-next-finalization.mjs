...
for(const width of widths){
 ...
 await page.locator('.dialog-favorite').click();await page.locator('#dialog-close').click();await page.locator('[data-action="favorites"]').click();assert(await page.locator('.product-card').count()===1,`${width}: favorite position`);await page.locator('.product-card .card-open').first().click();assert(await page.locator('.add-cart').getAttribute('data-cart-id')===selected,`${width}: favorite variant lost`);await page.locator('.add-cart').click();await page.locator('#dialog-close').click();await page.locator('[data-action="cart"]').click();
 ...
 if(width===320){
  ...
  const telegram=destinations.find(value=>value.startsWith('https://t.me/+79057267946?text='));assert(telegram&&!telegram.includes('/share/url'),'Telegram destination');
  ...
  assert(new URL(share).searchParams.get('order')===`${selected}.2`,'share payload');for(const pii of ['Тест Покупатель','999','Москва','Тестовый комментарий','postes@mail.ru'])assert(!share.includes(pii),`share contains PII ${pii}`);
  const recipient=await browser.newContext(),rp=await recipient.newPage();await rp.goto(base);await rp.evaluate(()=>localStorage.setItem('forma-next-cart',JSON.stringify({'276':3})));await rp.goto(share,{waitUntil:'networkidle'});
  assert(await rp.locator('#order-title').textContent()==='Заказ по ссылке','shared title');assert(await rp.evaluate(()=>localStorage.getItem('forma-next-cart'))===JSON.stringify({'276':3}),'shared overwrote cart');assert(await rp.locator('.order-item').getAttribute('data-order-item')===selected,'shared variant');assert((await rp.locator('.quantity b').textContent())==='2','shared quantity');
  await rp.locator('.save-shared').click();const saved=JSON.parse(await rp.evaluate(()=>localStorage.getItem('forma-next-cart')));assert(saved[selected]===2&&saved['276']===3,'explicit shared save');await recipient.close();
 }
 const footer=page.locator('.site-footer'),bodyText=normalize(await page.locator('body').innerText());
 assert(await footer.locator('a[href="tel:+79057267946"]').count()===1,`${width}: footer phone`);assert(await footer.locator('a[href="https://wa.me/79057267946"]').count()===1,`${width}: footer WhatsApp`);assert(await footer.locator('a[href="https://t.me/+79057267946"]').count()===1,`${width}: footer Telegram`);assert(await footer.locator('a[href="mailto:postes@mail.ru"]').count()===1,`${width}: footer email`);assert(normalize(await footer.innerText()).includes('MAX: +7 (905) 726-79-46'),`${width}: footer MAX`);assert(await footer.locator('a[href*="max.ru"]').count()===0,`${width}: fabricated MAX URL`);assert(bodyText.includes('Ежедневно, 10:00–20:00'),`${width}: hours`);
